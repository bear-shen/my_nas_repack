<template>
  ...
  >
    <template v-slot:browser_content>
      <template v-if="show">
        <div class="browser_content_media_alpha"
             @click="onStatus($event)"
        >
          <img v-if="item.file?.path_preview" :src="item.file?.path_preview">
          <span v-else class="listIcon listIcon_file_audio"></span>
        </div>
        <audio
    ...
    <template v-slot:btn_spec>
      ...
      <!--      <button v-if="fullscreen" :class="['sysIcon','sysIcon_arrowsalt',]" @click="onFullscreen()"></button>-->
      <!--      <button v-else :class="['sysIcon','sysIcon_shrink',]" @click="onFullscreen()"></button>-->
    </template>
  </browser-base>
</template>
...
@Options({
  ...
  methods: {
    initMeta: function () {
      ...
      const dom = (this.$refs.media as HTMLAudioElement);
      if (!dom) {
    ...
    onTimeChange: function (e: Event) {
      const vDom = (this.$refs.media as HTMLAudioElement);
      if (!vDom) return;
